...
fn read_reference(name: &str) -> std::io::Result<Vec<String>> {
  let project_path = Path::new(env!("CARGO_MANIFEST_DIR"));
  let reference_path =
    project_path.join(format!("tests/riscv-arch-test/rv64i_m/I/references/{}.reference_output", name));
...
macro_rules! generate_test_inst {
  ($($test_name:ident => $name:literal),*,) => {
    ...
    fn $test_name() {
      ...
      let mut cpu = Cpu::new(parse_result.entry_point, Isa::new("rv64i").unwrap(), bus, false);
